...
if ( ! function_exists( 'homey_get_price' ) ) {
    ...
    function homey_get_price() {
        ...
        $period_price = get_post_meta(get_the_ID(), $homey_prefix. 'custom_period', true);

        $day_date_price  = floatval( get_post_meta(get_the_ID(), $homey_prefix . 'day_date_price', true));
        ...
        $current_timestamp = new DateTime($current_date);
        $period_current =  $current_timestamp->getTimestamp();

        $test = $period_price[$period_current]['night_price'];

        if(isset($period_price[$period_current]) && isset( $period_price[$period_current]['night_price'] ) &&  $period_price[$period_current]['night_price']!=0 ){
            $price = $period_price[$period_current]['night_price'];
