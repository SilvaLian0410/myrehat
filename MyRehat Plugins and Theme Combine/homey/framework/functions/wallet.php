...
if(!function_exists('homey_add_earning')) {
    function homey_add_earning($reservation_id) {
        ...
        */
        ...
        $table_name = $wpdb->prefix . 'homey_earnings_date';

        $is_added = $wpdb->query(
            "SELECT * FROM $table_name WHERE user_id = $listing_host AND  guest_id = $listing_guest AND listing_id = $listing_id AND reservation_id = $reservation_id AND check_in_date = $check_in_date AND check_out_date = $check_out_date"
        );
...
if(!function_exists('homey_add_exp_earning')) {
    function homey_add_exp_earning($reservation_id) {
        ...
        //deduct services fee and security deposit from total
        $sf_and_sd = (float)$services_fee + (float)$security_deposit;

        //chargeable amount for host fee
        $chargeable_amount = (float)$total_price - (float)$sf_and_sd;

        //Host fee
        $host_fee = (float)($host_fee_percent / 100) * (float)$chargeable_amount;
        ...
        */
        $net_earnings = (float)$upfront_payment - (float)$services_fee; //deduct services fee from $upfront
        $net_earnings = (float)$net_earnings - (float)$host_fee; //deduct host fee from upfront_payment
        $net_earnings = (float)$net_earnings - (float)$security_deposit; //deduct security deposit

        // Julian - On top of this Code I added (float) to the variabes.

        $table_name = $wpdb->prefix . 'homey_earnings_date';

        $is_added = $wpdb->query(
            "SELECT * FROM $table_name WHERE user_id = $experience_host AND  guest_id = $experience_guest AND listing_id = $experience_id AND reservation_id = $reservation_id AND check_in_date = $check_in_date AND check_out_date = $check_out_date"
        );
...
//Cancel or Refund by Guests

if(!function_exists('homey_cancel_earning')) {
    function homey_cancel_earning($reservation_id) {
        global $wpdb;
        $allowded_html = array();
        ...
        // Delete the row based on the reservation ID
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM $table_name WHERE reservation_id = %d",
                $reservation_id
            )
        );
    }
...
if(!function_exists('homey_cancel_exp_earning')) {
    function homey_cancel_exp_earning($reservation_id) {
        global $wpdb;
        $allowded_html = array();
