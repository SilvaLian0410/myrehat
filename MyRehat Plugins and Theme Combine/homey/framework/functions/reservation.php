...
function applied_coupon_discount($total_price){

    if (WC()->cart->has_discount()) {
        // Get an array of applied coupon codes.
        $applied_coupons = WC()->cart->get_applied_coupons();

        // Loop through each applied coupon to get its type and amount.
        foreach ($applied_coupons as $coupon_code) {
            // Get the coupon object using its code.
            $coupon = new WC_Coupon($coupon_code);

            // Get the coupon type (e.g., 'fixed_cart' or 'percent').
            $coupon_type = $coupon->get_discount_type();

            if ($coupon_type === 'fixed_cart') {
                $discount_amount = floatval(wc_price($coupon->get_amount())); // Convert to float
                $total_price -= (float)$discount_amount;
            } elseif ($coupon_type === 'percent') {
                $percent_discount = $coupon->get_amount();
                // Calculate the discount amount based on the percentage and update the total price.
                $discount_amount = (float)($percent_discount / 100) * (float)$total_price;
                $total_price -= (float)$discount_amount;
            }
        }
    }
...
function applied_coupon_ammount($total_price){
...
if( !function_exists('homey_add_reservation') ) {
    function homey_add_reservation() {
        ...
        if($current_user->ID == 0 && $no_login_needed_for_booking == "yes" && isset($_REQUEST['new_reser_request_user_email'])) {
            ...
            if(empty($email)){
                echo json_encode(
                    array(
                        'success' => false,
                        'message' => esc_html__('You need to login in order to make a reservation', 'homey')
                    )
                );
                wp_die();
            }
        ...
        if($is_available) {
            ...
            //added new variables for sending email
            $check_in_booking = get_post_meta($reservation_id, 'reservation_checkin_date', true);
            $check_out_booking = get_post_meta($reservation_id, 'reservation_checkout_date', true);
            $price_booking = get_post_meta($reservation_id, 'reservation_total', true);
            $listing_id_booking = get_post_meta($reservation_id, 'reservation_listing_id', true);
            $adult_guest = get_post_meta($reservation_id, 'reservation_adult_guest', true);
            $children_guest = get_post_meta($reservation_id, 'reservation_child_guest', true);

            $email_args = array(
                'reservation_detail_url' => reservation_detail_link($reservation_id),
                'guest_message' => $guest_message,
                'listing_id' => $listing_id_booking,
                'listing_title'  =>  get_the_title($listing_id_booking),
                'message_link' => $message_link,
                'check_in' => $check_in_booking,
                'check_out' => $check_out_booking,
                'price' => $price_booking,
                'adults' => $adult_guest,
                'children' => $children_guest
            );
...
if( !function_exists('homey_add_instance_booking') ) {
    function homey_add_instance_booking($listing_id, $check_in_date, $check_out_date, $guests, $renter_message, $extra_options, $user_id = null, $adult_guest=0, $child_guest=0) {
        ...
        $discount_woocommerce = applied_coupon_ammount($total_price);
        //Test Updates Data;
        $discount_data = array(
            array(
                'discount_name' => 'MyRehat Discount :',
                'discount_value' =>  $discount_woocommerce,
            )
        );
        
        update_post_meta($reservation_id, 'homey_reservation_discount', $discount_data);
    ...
    function homey_get_extra_discount($reservation_id) {
        ...
        if(!empty($discount_meta)) {
            foreach($discount_meta as $discount) {
                ...
                $total_discount = (float)$total_discount + (float)$discount_value;
...
//             echo ' the day => price='. $returnPrice.'<br>';
        ...
        $total_price = $total_price - $price_per_night + 5;
...
if(!function_exists('homey_decline_reservation')) {
    function homey_decline_reservation() {
        ...
        // If Experience Owner
		if ($listing_id == 0 && is_null($listing_id)) {
            $experience_id = get_post_meta($reservation_id, 'reservation_id', true);
            ...
            if( $experience_owner != $userID && !homey_is_admin() ) {
                ...
                echo json_encode(
                    ...
                    // Send to Reservation Experiences Detail Page
                    ...
                    $email_args = array('reservation_detail_url' => $reservation_detail_link );
                     ...
                     homey_email_composer( $renter_email, 'declined_exp_reservation', $email_args );
                     //        $admin_email = get_option( 'admin_email' );
...
if(!function_exists('homey_cancelled_reservation')) {
    function homey_cancelled_reservation() {
		...
		if ($listing_id == 0 && is_null($listing_id)) {
            ...
            $check_in_date     = strtotime(get_post_meta($reservation_id, "reservation_checkin_date", true));
    
            // Get the check-in date for the reservation
            $current_date = date('Y-m-d', strtotime('now'));
    
           // Compare current date with check-in date
           if ($current_date <= $check_in_date) {
            // You cannot cancel after your check-in day
            echo json_encode(array(
                'success' => false,
                'message' => "You cannot cancel your booking during or after Check-In"
            ));
             wp_die();
            }

             // Get the cancellation policy ID for the listing
             $cancellation_policy_id = homey_get_experience_data_reservation('cancellation_policy', $experience_id);

              // Check cancellation policy based on days difference and cancellation policy ID
              if (!empty($cancellation_policy_id) && $cancellation_policy_id != '') {
                $cancellation_policy_content   = get_the_content( '', '',  $cancellation_policy ); // Where $cancellation_policy is the ID
                
                $message = "You cannot cancel this reservation:\n" . $cancellation_policy_content . "contact MyRehat support if you think this is a mistake";
            ...
            if( !homey_is_admin() && ($experience_renter != $userID) && ($experience_owner != $userID) ) {
                echo json_encode(
                    array(
                        'success' => false,
            ...
            homey_email_composer( $to_email, 'cancelled_exp_reservation', $email_args );
    //        $admin_email = get_option( 'admin_email' );
    //        homey_email_composer( $admin_email, 'cancelled_reservation', $email_args );
        ...
        $check_in_date     =  date('Y-m-d', strtotime(get_post_meta($reservation_id, "reservation_checkin_date", true)));
        ...
        if ($current_date >= $check_in_date) {
            ...
            echo json_encode(array(
                'success' => false,
                'message' => "You cannot cancel your booking during or after Check-In : " .  $check_in_date
             ));
            wp_die();
        }

        // Get the cancellation policy ID for the listing
        $cancellation_policy_id = homey_get_listing_data_reservation('cancellation_policy', $listing_id);
       ...
       if (!empty($cancellation_policy_id) && $cancellation_policy_id != '') {
        ...
        if(!homey_is_admin() && (($listing_renter != $userID) && ($listing_owner != $userID)) ) {
            echo json_encode(
                array(
                    'success' => false,
                    'message' => $local['listing_renter_text']
...
if(!function_exists('homey_save_extra_expenses')) {
    function homey_save_extra_expenses() {
        ...
        // If Experience Owner, It will redirect to Experience Reservation Page
