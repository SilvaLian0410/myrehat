...
if( !function_exists('homey_add_exp_reservation') ) {
    function homey_add_exp_reservation() {
        ...
        if($is_available) {
            ...
            $experience_id_booking = get_post_meta($reservation_id, 'reservation_experience_id', true);
            ...
            $email_args = array(
                ...
                'experience_id' => $experience_id_booking,
                'experience_title'  =>  get_the_title($experience_id_booking),
            ...
            homey_email_composer( $owner_email, 'new_exp_reservation', $email_args );
            homey_email_composer( $admin_email, 'admin_booked_exp_reservation', $email_args );
            ...
            if(isset($current_user->user_email)){
                ...
                homey_email_composer( $current_user->user_email, 'new_exp_reservation_sent', $email_args );
            }
...
//             echo ' the person => price='. $returnPrice.'<br>';
        ...
        if($enable_services_fee == 1 && $offsite_payment != 1) {
            $services_fee_type  = homey_option('exp_services_fee_type');
...
if(!function_exists('homey_confirm_exp_reservation')) {
    function homey_confirm_exp_reservation() {
        ...
        if($no_upfront == 'no_upfront') {
            ...
            $email_args = array('reservation_detail_url' => $reservation_detail_link );
            homey_email_composer( $renter_email, 'confirm_exp_reservation', $email_args );
        }
    ...
    function homey_get_exp_reservation_action($status, $upfront_payment, $payment_link, $ID, $class) {
        ...
        if(homey_experience_guest($ID)) {
            ...
            if($status == 'under_review') {
                ...
                $action .= '<button class="btn btn-grey-light '.esc_attr($class).'" data-toggle="collapse" id="cancel-exp-reservation-btn" data-target="#cancel-exp-reservation" aria-expanded="false" aria-controls="collapseExample">'.esc_html__('Cancel', 'homey').'</button>';
                ...
                $action .= '<button class="btn btn-grey-light btn-full-width" data-toggle="collapse" id="cancel-exp-reservation-btn" data-target="#cancel-exp-reservation" aria-expanded="false" aria-controls="collapseExample">'.esc_html__('Cancel', 'homey').'</button>';
            ...
            if($status == 'under_review') {
                ...
                $action .= '<button class="btn btn-grey-light '.esc_attr($class).'" data-toggle="collapse" id="decline-exp-reservation-btn" data-target="#decline-reservation" aria-expanded="false" aria-controls="collapseExample">'.esc_html__('Decline', 'homey').'</button>';
...
if(!function_exists('homey_exp_reservation_mark_paid')) {
    function homey_exp_reservation_mark_paid() {
        if(homey_is_admin() || homey_is_host()){
            ...
            homey_email_composer( $renter_email, 'booked_exp_reservation', $email_args );
            homey_email_composer( $owner_email, 'booked_exp_reservation', $email_args );
            homey_email_composer( $admin_email, 'admin_booked_exp_reservation', $email_args );
