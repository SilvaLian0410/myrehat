...
$dashboard_reservations = homey_get_template_link_dash('template/dashboard-reservations.php');//guest booking
$dashboard_host_reservations = homey_get_template_link_dash('template/dashboard-reservations2.php');//host gets booking
...
<nav id="user-nav" class="nav-dropdown main-nav-dropdown collapse navbar-collapse">
    <ul>
        ...
        if( !empty($dashboard_profile) ) {
            ...
            if(!homey_is_renter())  {
            echo '<li>
            <a href="'.esc_url($dashboard_listings).'">
                <i class="homey-icon homey-icon-layout-agenda-interface-essential"></i> '.$homey_local['m_listings_label'].'
            </a>';
            echo '<ul>';
                    echo '<li><a href="'.esc_url($dashboard_add_listing).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('Add New', 'homey').'</a></li>';
				
                    if(!empty($dashboard_reservations) ){
                        $new_notification = homey_booking_notification(1);
                        $new_notification = $new_notification > 0 ? '<span class="msg-alert" style="display: block;"></span>' : '<span class="msg-alert" style="display: none;"></span>';    
                        
                        echo '<li><a href="'.esc_url($dashboard_reservations).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('My Bookings', 'homey').' '.$new_notification.'</a></li>';
                        //letak notification function untuk booking
                    }

                echo '<li><a href="'.esc_url($dashboard_host_reservations).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('Reservation', 'homey').'</a></li>';
            ...
            if(!homey_is_renter()){
                echo '<li>
            <a href="'.esc_url($dashboard_experiences).'">
                <i class="homey-icon homey-icon-layout-agenda-interface-essential"></i> '.$homey_local['m_experiences_label'].'
            </a>';
            echo '<ul>';
                    echo '<li><a href="'.esc_url($dashboard_add_experience).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('Add New', 'homey').'</a></li>';
				if(!empty($dashboard_reservations) ){
                    $new_notification = homey_booking_notification(1);
                    $new_notification = $new_notification > 0 ? '<span class="msg-alert" style="display: block;"></span>' : '<span class="msg-alert" style="display: none;"></span>';    
                    
                    echo '<li><a href="'.esc_url($dashboard_exp_reservations).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('My Bookings', 'homey').' '.$new_notification.'</a></li>';
                    //letak notification function untuk booking
                }
                echo '<li><a href="'.esc_url($dashboard_exp_host_reservations).'"><i class="homey-icon homey-icon-arrow-right-1"></i> '.esc_html__('Reservation', 'homey').'</a></li>';
        ...
        if( !empty($dashboard_reservations) ) {
            ...
            if(homey_is_renter()) {
                echo '<li>
				 ...
				 echo '<li>
                <a href="'.esc_url($dashboard_exp_reservations).'"><i class="homey-icon homey-icon-calendar-3"></i>  '.esc_html__('Experience Reservation', 'homey').'</a>
            </li>';
            }
        }
