...
//Add for defining the listing
if (!function_exists('homey_get_listing_data_reservation')) {
    function homey_get_listing_data_reservation($field, $id)
    {
        $prefix = 'homey_';
        $post_id = $id;
...
if (!function_exists('homey_listing_gallery_upload')) {
    function homey_listing_gallery_upload()
    {
        ...
        $submitted_file = $_FILES['listing_upload_file'];
         if ($submitted_file['size'] > $maxfilesize) {
            $reason = esc_html__('File Size Exceeds Maximum Size', 'homey');
        ...
        if (isset($uploaded_image['file']) && $maxfilesize != -1) {
